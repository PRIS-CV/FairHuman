...
def safe_get_box_mask_keypoints(image, retinaface_result, crop_ratio, face_seg, mask_type):
    ...
    if len(retinaface_result) != 0:
        ...
        for index in range(len(retinaface_result)):
            # 获得retinaface的box并做扩充
            retinaface_box      = np.array(retinaface_result[index])
...
def call_face_crop(retinaface_result, image, crop_ratio, prefix="tmp"):
    # retinaface detect 
    # get mask and keypoints
    retinaface_box   = safe_get_box_mask_keypoints(image, retinaface_result, crop_ratio, None, "crop")

    return retinaface_box
...
class Face_Skin(object):
    ...
    # index => label
    # 1:'整体皮肤', 2:'右眉毛', 3:'左眉毛', 4:'右眼睛', 5:'左眼睛', 6:'眼窝', 7:'右耳', 
    # 8:'右耳', 10:'鼻子', 12:'上嘴唇', 13:'下嘴唇', 14: 脖子， 16：肩膀，17 头发
    def __call__(self, image, retinaface_result, needs_index=[[12, 13]], revse=False):
        # needs_index 12, 13 means seg the lip
        with torch.no_grad():
            total_mask = np.zeros_like(np.uint8(image))
            
            # detect image
            # retinaface_boxes = call_face_crop(retinaface_result, image, 1.5, prefix="tmp")
            # retinaface_box = retinaface_boxes[0]
            retinaface_box = retinaface_result[0]
            ...
            for _needs_index in needs_index:
                if revse == True:
                    total_mask = np.ones_like(np.uint8(image)) * 255
                else:
                    total_mask = np.zeros_like(np.uint8(image))
                ...
                sub_mask = np.clip(sub_mask, 0, 1)

                if revse == True:
                    sub_mask = np.where(sub_mask == 1, 0, 1)
