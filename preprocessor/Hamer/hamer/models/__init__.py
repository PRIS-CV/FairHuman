...
def load_hamer(checkpoint_path=DEFAULT_CHECKPOINT):
    ...
    # revise
    model_cfg.defrost()
    model_cfg.MANO.DATA_DIR = str(Path(checkpoint_path).parent.parent.parent / 'data')
    model_cfg.MANO.MODEL_PATH = str(Path(checkpoint_path).parent.parent.parent / 'data/mano')
    model_cfg.MANO.MEAN_PARAMS = str(Path(checkpoint_path).parent.parent.parent / 'data/mano_mean_params.npz')
    model_cfg.freeze()
