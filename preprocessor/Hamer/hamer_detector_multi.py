...
class HamerDetector:
    ...
    def __call__(self, image: Image.Image, bbox_scale_factor, mask_scale_factor, is_cropped):
        ...
        if yolo_detections is None:
            return bboxes_padded, multi_conditions, masks, None
        ...
        wholebody_pose = self.dwpose(img)
        #canv = np.zeros(shape=(int(image.size[0]), int(image.size[1]), 3), dtype=np.uint8)
        #global facepose
        #for vitposes in vitposes_out:
        #    face = []
        #    for i in range(23,91):
        #        face.append([vitposes['keypoints'][i][0],vitposes['keypoints'][i][1]])
        #    #print(face)
        #    facepose = draw_facepose(canv, face)
        #    facepose = cv2.cvtColor(facepose, cv2.COLOR_BGR2RGB)
        #    canv = facepose
        #facepose = cv2.cvtColor(facepose, cv2.COLOR_BGR2RGB)
        #cv2.imwrite('face.jpg',facepose)
        ...
        if bboxes == []:
            return bboxes_padded, multi_conditions, masks, wholebody_pose
        ...
        for box in bboxes:
            padding = int(
                0.2 * max(box[2] - box[0], box[3] - box[1]))
            bbox_padded = bbox_padding(
                box, padding
            )
            bboxes_padding.append(bbox_padded)
        for i in range(len(bboxes)):
            for j in range(len(bboxes)):
                if j == i:
                    continue
                if 0 < calculate_iou(bboxes_padding[i], bboxes_padding[j]) < 1:
                    merged_index.append((min(i, j), max(i, j)))
                    break
        merged_index = np.unique(merged_index)
        for index in merged_index:
            xmin, ymin, xmax, ymax = bboxes[index]
            merged_is_rights.append(is_right[index])
            merged_boxes.append([xmin, ymin, xmax, ymax])
        merged_boxes_new = group_rectangles(merged_boxes, 256, 256)
        merged_is_rights_new = []
        for merged_box_new in merged_boxes_new:
            merged_is_right_new = []
            for merged_box_new1 in merged_box_new:
                for k, bbox in enumerate(bboxes):
                    if (bbox == merged_box_new1).all():
                        merged_is_right_new.append(is_right[k])
            merged_is_rights_new.append(merged_is_right_new)
        # 控制bboxes上限
        # extra_delete_index = []
        # if len(bboxes) - len(merged_index) + len(merged_boxes_new) > 4:
        #     areas = []
        #     for k, bbox in enumerate(bboxes):
        #         if k in merged_index:
        #             continue
        #         area = calculate_area(bbox)
        #         areas.append(area)
        #     areas_invalid = sorted(areas)[:len(areas) - 4 + len(merged_boxes_new)]
        #     for k, bbox in enumerate(bboxes):
        #         if k in merged_index:
        #             continue
        #         area = calculate_area(bbox)
        #         if area in areas_invalid:
        ...
        if is_cropped:
            # 单手部分
            for k, (bbox, right) in enumerate(zip(bboxes, is_right)):
                if k in merged_index:
                    continue
                bbox_padded, bbox_relative = scale_to_square(bbox, bbox_scale_factor)
                multi_condition = self.inference(
                    image.crop(bbox_padded),
                    bbox_relative,
                    right,
                    is_cropped
                )
                multi_conditions.append(multi_condition)
                patches.append(image.crop(bbox_padded))
                bboxes_padded.append(bbox_padded)
                bbox_from_multi = get_bounding_box(multi_condition)
                bbox_for_mask = [
                ...
                masks.append(mask)
            # 合并部分
            if len(merged_boxes_new) > 0:
                for merged_box_new, merged_is_right_new in zip(merged_boxes_new, merged_is_rights_new):
                    bbox_crop = merge_rectangles(merged_box_new)
                    bbox_padded, _ = scale_to_square(bbox_crop, bbox_scale_factor)
                    new_x1, new_y1, new_x2, new_y2 = bbox_padded
                    if new_x1 < 0:
                        new_x1 = 0
                    if new_y1 < 0:
                        new_y1 = 0
                    ...
                    if new_y2 > 1024:
                        new_y2 = 1024
                    bbox_padded = (new_x1, new_y1, new_x2, new_y2)
                    merged_box_relative = []
                    for box in merged_box_new:
                        x1, y1, x2, y2 = box
                        bbox_relative = (abs(x1 - new_x1), abs(y1 - new_y1), abs(x2 - new_x1), abs(y2 - new_y1))
                        merged_box_relative.append(bbox_relative)
                    multi_condition = self.inference(
                        image.crop(bbox_padded),
                        merged_box_relative,
                        merged_is_right_new,
                    ...
                    # image.crop(bbox_padded).save('patch.png')
                    # multi_condition.save('multi_condition.png')
                    # import pdb;pdb.set_trace()
                    multi_conditions.append(multi_condition)
                    patches.append(image.crop(bbox_padded))
                    bboxes_padded.append(bbox_padded)
                    merged_mask = Image.fromarray(
                        np.zeros((image.crop(bbox_padded).height, image.crop(bbox_padded).width))).convert('L')
                    merged_global_mask = Image.fromarray(np.zeros((image.size[0], image.size[1]))).convert('L')
                    for bbox in merged_box_relative:
                        bbox_padded_m, bbox_relative = scale_to_square(bbox, bbox_scale_factor)
                        crop_multi_condition = multi_condition.crop(bbox_padded_m)
                        bbox_from_multi = get_bounding_box(crop_multi_condition)
                        bbox_for_mask = [
                            min(bbox[0], bbox_from_multi[0] + bbox_padded_m[0]),
                            min(bbox[1], bbox_from_multi[1] + bbox_padded_m[1]),
                            max(bbox[2], bbox_from_multi[2] + bbox_padded_m[0]),
                            max(bbox[3], bbox_from_multi[3] + bbox_padded_m[1]),
                        ]
                        mask = create_mask_from_bbox(scale_rectangle(bbox_for_mask, mask_scale_factor), image.size)
                        ...
                        merged_mask.paste(crop_mask, [xmin, ymin, xmax, ymax])
                    merged_global_mask.paste(merged_mask, bbox_padded)
                    masks.append(merged_global_mask)
                # import pdb;pdb.set_trace()
        else:
            if len(bboxes)==0:
                return bboxes_padded, multi_condition, masks, wholebody_pose
            multi_condition= self.inference(
                image,
                bboxes,
                is_right,
            ...
            multimap = wholebody_pose.copy()
            multi_array = np.array(multi_condition)
            nonzero_y, nonzero_x, _ = (multi_array != 0).nonzero()
            multimap[nonzero_y, nonzero_x, :] = multi_array[nonzero_y, nonzero_x, :]
            multi_condition = Image.fromarray(np.uint8(multimap))
            multi_conditions.append(multi_condition)
