...
class HamerDetector:
    ...
    def __call__(self, image: Image.Image, bbox_scale_factor, mask_scale_factor, is_cropped):
        ...
        if yolo_detections is None:
            return bboxes_padded, depth_conditions, pose_conditions, multi_conditions, mesh_conditions, masks, None

        if len(yolo_detections) == 0:
            return bboxes_padded, multi_conditions, depth_conditions, pose_conditions, mesh_conditions, masks, None
        ...
        if bboxes == []:
            return bboxes_padded, multi_conditions, depth_conditions, pose_conditions, mesh_conditions, masks, wholebody_pose
    ...
    def inference(self, patch: Image.Image, dw_pose, bbox, right, is_cropped):
        ...
        multimap = dw_pose.copy()
        depthmap = dw_pose.copy()
        posemap = dw_pose.copy()
        meshmap = dw_pose.copy()
        ...
        depth_array = np.array(depth_condition)
        nonzero_y, nonzero_x, _ = (depth_array != 0).nonzero()
        depthmap[nonzero_y, nonzero_x, :] = depth_array[nonzero_y, nonzero_x, :]
        depth_condition = Image.fromarray(np.uint8(depthmap))

        pose_array = np.array(pose_condition)
        nonzero_y, nonzero_x, _ = (pose_array != 0).nonzero()
        posemap[nonzero_y, nonzero_x, :] = pose_array[nonzero_y, nonzero_x, :]
        pose_condition = Image.fromarray(np.uint8(posemap))
        ...
        if len(all_verts) > 0:
            ...
            mesh_array = np.array(mesh_condition)
            nonzero_y, nonzero_x, _ = (mesh_array != 0).nonzero()
            meshmap[nonzero_y, nonzero_x, :] = mesh_array[nonzero_y, nonzero_x, :]
            mesh_condition = Image.fromarray(np.uint8(meshmap))
