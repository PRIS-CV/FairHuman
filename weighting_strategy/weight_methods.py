...
class WeightMethod:
    def __init__(self, n_tasks: int, device: torch.device, max_norm = 1.0, accelerator=None):
        super().__init__()
...
class FairGrad(WeightMethod):
    def __init__(self, n_tasks, device: torch.device, alpha=1.0, max_norm=1.0, accelerator=None):
        super().__init__(n_tasks, device=device, accelerator=accelerator)
        self.alpha = alpha
        self.max_norm = max_norm
    ...
    def get_weighted_loss(
        ...
        **kwargs,
        ...
        for i in range(self.n_tasks):
            if i < self.n_tasks - 1:
                self.accelerator.backward(losses[i],retain_graph=True)
                # losses[i].backward(retain_graph=True)
            else:
                self.accelerator.backward(losses[i])
                # losses[i].backward()
            self.grad2vec(shared_parameters, grads, grad_dims, i)
            # multi_task_model.zero_grad_shared_modules()
            for p in shared_parameters:
    ...
    def overwrite_grad(self, shared_parameters, newgrad, grad_dims):
        ...
        for param in shared_parameters:
            ...
            param.grad = this_grad.data.clone().to(dtype=param.dtype)
            cnt += 1
